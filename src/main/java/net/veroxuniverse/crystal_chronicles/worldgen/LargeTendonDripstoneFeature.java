...
public class LargeTendonDripstoneFeature extends Feature<LargeMuscleDripstoneConfiguration> {
    public LargeTendonDripstoneFeature(Codec<LargeMuscleDripstoneConfiguration> pCodec) {
        super(pCodec);
    }
        ...
        if (!MuscleUtils.isEmptyOrWater(worldgenlevel, blockpos)) {
            ...
            if (!optional.isEmpty() && optional.get() instanceof Column.Range) {
                ...
                if (column$range.height() < 4) {
                    ...
                    LargeTendonDripstoneFeature.LargeDripstone largedripstonefeature$largedripstone = makeDripstone(
                    ...
                    LargeTendonDripstoneFeature.LargeDripstone largedripstonefeature$largedripstone1 = makeDripstone(
                    ...
                    LargeTendonDripstoneFeature.WindOffsetter largedripstonefeature$windoffsetter;
                    if (largedripstonefeature$largedripstone.isSuitableForWind(largedripstoneconfiguration)
                        ...
                        largedripstonefeature$windoffsetter = new LargeTendonDripstoneFeature.WindOffsetter(
                                blockpos.getY(), randomsource, largedripstoneconfiguration.windSpeed
                        );
                    } else {
                        largedripstonefeature$windoffsetter = LargeTendonDripstoneFeature.WindOffsetter.noWind();
                    }
    ...
    private static LargeTendonDripstoneFeature.LargeDripstone makeDripstone(
        ...
        return new LargeTendonDripstoneFeature.LargeDripstone(
                pRoot, pPointingUp, pRadius, (double)pBluntnessBase.sample(pRandom), (double)pScaleBase.sample(pRandom)
        );
    }

    private void placeDebugMarkers(WorldGenLevel pLevel, BlockPos pPos, Column.Range pRange, LargeTendonDripstoneFeature.WindOffsetter pWindOffsetter) {
        ...
        for (BlockPos.MutableBlockPos blockpos$mutableblockpos = pPos.atY(pRange.floor() + 2).mutable();
            ...
            if (MuscleUtils.isEmptyOrWater(pLevel, blockpos) || pLevel.getBlockState(blockpos).is(CCBlocks.TENDON_BLOCK.get())) {
    ...
    static final class LargeDripstone {
        ...
        boolean moveBackUntilBaseIsInsideStoneAndShrinkRadiusIfNecessary(WorldGenLevel pLevel, LargeTendonDripstoneFeature.WindOffsetter pWindOffsetter) {
            while (this.radius > 1) {
        ...
        void placeBlocks(WorldGenLevel pLevel, RandomSource pRandom, LargeTendonDripstoneFeature.WindOffsetter pWindOffsetter) {
                        ...
                        if (k > 0) {
                            ...
                            for (int i1 = 0; i1 < k && blockpos$mutableblockpos.getY() < l; i1++) {
                                ...
                                if (MuscleUtils.isEmptyOrWaterOrLava(pLevel, blockpos)) {
                                    flag = true;
                                    Block block = CCBlocks.TENDON_BLOCK.get();
    ...
    static final class WindOffsetter {
        ...
        static LargeTendonDripstoneFeature.WindOffsetter noWind() {
            return new LargeTendonDripstoneFeature.WindOffsetter();
        }
